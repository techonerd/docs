...
class _FileLocation(object):
  ...
  def __init__(
    ...
    if self._start_line and 'github.com' in self.url:
...
class ReferenceResolver(object):
  ...
  def from_visitor(cls, visitor, **kwargs):
    ...
    for full_name, obj in visitor.index.items():
      ...
      if (obj_type not in (ObjType.CLASS, ObjType.MODULE)
          and obj_type in (ObjType.CALLABLE, ObjType.TYPE_ALIAS)
          and is_class_attr(full_name, visitor.index) or obj_type not in (
              ObjType.CLASS,
              ObjType.MODULE,
              ObjType.CALLABLE,
              ObjType.TYPE_ALIAS,
  ...
  def _one_ref(self, match, relative_path_to_root, full_name=None):
    ...
    try:
      ...
      is_python = any(
          string == py_module_name or string.startswith(py_module_name + '.')
...
def _get_other_member_doc(
  ...
  description = None
  if my_doc != class_doc and my_doc is not None:
    description = my_doc
...
class ClassPageInfo(PageInfo):
  ...
  def _add_method(
    ...
    elif member_info.short_name == '__new__' or not isinstance(
          original_method, staticmethod):
